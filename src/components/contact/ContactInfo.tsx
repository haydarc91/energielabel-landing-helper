...
const ContactInfo = ({ calculatedPrice, formData, addressDetails }: ContactInfoProps) => {
  // Time-based online status for WhatsApp (9:00-22:00)
  const now = new Date();
  const currentHour = now.getHours();
  const isWhatsAppOnline = currentHour >= 9 && currentHour < 22;
    ...
    <div className="lg:col-span-1">
      <div className="glass-card rounded-xl p-6 md:p-8 sticky top-24">
        ...
        <div className="space-y-4 text-gray-600 mb-6">
          ...
          {/* WhatsApp button with online status */}
          <a 
            href="https://wa.me/31612345678" 
            target="_blank" 
            rel="noopener noreferrer" 
            className="flex items-center gap-2 mt-2 text-gray-700 hover:text-green-600 transition-colors"
          >
            <div className="relative">
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512" className="w-5 h-5 fill-green-500">
            ...
            <span>WhatsApp {isWhatsAppOnline ? 'Online' : 'Offline'}</span>
          </a>
        </div>
